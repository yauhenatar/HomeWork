...
"""
Напишите класс Money, принимающий два аргумента - dollars и cents. С помощью этих атрибутов необходимо 
создать ещё один атрибут - total_cents, в котором должно находиться общее значение центов, рассчитанное 
из атрибутов dollars и cents (1 доллар = 100 центов).
У класса должны быть:
    геттер, который возвращает количество имеющихся долларов;
    сеттер, который принимает целое неотрицательное число - количество долларов, с помощью которого 
рассчитывается новое значение атрибута total_cents, при этом значение центов должно оставаться прежним.
Если в сеттер передано число, не удовлетворяющее условию, то печатается сообщение "Error dollars".
    геттер cents, который возвращает количество имеющихся центов;
    сеттер cents, который принимает целое неотрицательное число меньшее 100 - количество центов, с 
помощью которого рассчитывается новое значение атрибута total_cents, при этом значение долларов должно 
оставаться прежним. Если в сеттер передано число, не удовлетворяющее условию, то печатается сообщение
"Error cents";
    Метод __str__, который возвращает строку в виде "Ваше состояние составляет {dollars} долларов 
{cents} центов".
"""
